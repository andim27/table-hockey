	...
	private var shayba_dx:int;//--расстояние от центра катка до шайбы по х
		...
		override public function analyze():void  {
			...
			*/
			switch (shayba_zone) {
				case 22: //--TEAM_2				
				  sendCommand("rotateY",{angle:190});	
				  sendCommand("moveRight");
				break;
				case 20://--TEAM_2	
				  sendCommand("rotateY",{angle:-10});	
				  sendCommand("moveLeft");
				break;
				case 21://--CENTER
				  sendCommand("rotateY",{angle:0});	
				  sendCommand("moveStop");
				break;
		...
		override public function sendCommand(cmd:String="",param:Object=null):void  {
			...
			//s_conf.main.team_2.manCommand(VRATAR_NOMER,cmd);
			body_obj.command(cmd,param);
			// code
		}
		private function where_is_shayba():void  {//---вычислить в какой зоне шайба
		  if ((cur_state != STATE_STOP)&&(s_conf.main.shayba.model != null)) {
